...
public class PermissionsUtils {
    ...
    public void checkRuntimePermissions(String[] permission){
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && null != permission) {
            ...
            Logger.d("checkRuntimePermissions() " + deniedPermissions.size());
            if(deniedPermissions.size() > 0){
                ...
                if(null != ISuccessCallBack){
                    ISuccessCallBack.onApplyPermissionsSuccess();
                }
            }
        }
    }
        ...
        if(null != permission) {
            for (String value : permission) {
                if (ContextCompat.checkSelfPermission(getActivity(), value)
                    ...
                    //没有权限 就添加
                    Logger.i( "findDeniedPermissions() " + value);
    ...
    private boolean shouldShowPermissions(String[] permission){
        ...
        for (String value : permission){
            ...
            Logger.i("shouldShowPermissions() " + value + " bRet=" + bRet);
            if (bRet){
                break;
            }
        }
        Logger.i("shouldShowPermissions(E) bRet=" + bRet);
        return bRet;
    }
    ...
    private void requestRuntimePermissions(String[] permission){
        if(shouldShowPermissions(permission)){
            Log.i(TAG, "requestRuntimePermissions() " + ISuccessCallBack);
            //提示用户需要授权
            showApplySnackbar(permission);
            if(null != ISuccessCallBack){
                ISuccessCallBack.onApplyPermissionsFail();
            }
        }else {
            //直接显示申请界面
    ...
    //TODO 显示权限申请界面
    public void showApplyDialog(String[] permission){
        Logger.i("showApplyDialog() ");
    ...
    private void showApplySnackbar(final String[] permission){
        try {
            ...
            Logger.i("showApplySnackbar() " + view);
            ...
            Toast.makeText(getActivity(),R.string.str_no_permissions_tip, Toast.LENGTH_LONG).show();
        }
    ...
    private void showGoSettingSnackbar(){
        try {
            View view = getActivity().getWindow().getDecorView().findViewById(android.R.id.content);
            Logger.i("showGoSettingSnackbar() " + view);
            ...
            Logger.e(e.getMessage());
            Toast.makeText(getActivity(),R.string.str_set_permissions_tip, Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void showSuccessSnackbar(){
        try {
            View view = getActivity().getWindow().getDecorView().findViewById(android.R.id.content);
            Logger.i("showSuccessSnackbar() " + view);
            ...
            Logger.e(e.getMessage());
            Toast.makeText(getActivity(),R.string.str_runtime_permission_succ, Toast.LENGTH_SHORT).show();
        }
    }
